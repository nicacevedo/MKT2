...
- F1: pues considera las métricas de Recall y Precision, lo que nos entrega una información más completa.


Algunas variables que no incorporaremos, son "Hue" o el matiz del vino, pues observando el EDA, notamos que para el tipo de vino 1 y 2 son bastantes similares, y podría llevar a confundir nuestros modelos al momentos de predecir.
...
Observando la matriz de confusión de cada modelo, podemos observar que en ambos casos el balanced accuracy es alto, por lo que los modelos estarían prediciendo bien el tipo de vino, presentando una cantidad ínfima de falsos negativos y positivos. Lo mismo ocurre al observar los valores de las métricas escogidas (F1 y Accuracy), siendo en todos los casos, sobre el 80%. 

Sobre la presencia de over/underfitting, podemos notar que los sets de entrenamiento y testeo fueron divididos en 0.56 train y 0.44 test, por lo que de partida, sería una buena distribución de los datos. Además, tanto las métricas para train como para test dan bastante bien, por lo que no habría presencia de overfitting ni underfitting.
...
```
Ambos modelos funcionan bastante bien, sin embargo, el modelo KNN es el que presenta mejores métricas (aunque por poco), presentando mejor accuracy (89%) y mejor F1 (87%).

Podemos resumirlo en la siguiente tabla:

|          | KNN       | SVM       |
|----------|-----------|-----------|
| F1       | 0.8666667 | 0.8400000 |
| Accuracy | 0.8846154 | 0.8461538 |
