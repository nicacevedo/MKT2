...
En base al EDA anteriormente desarrollada se eligen como atributos asociados a las alternativas: Combat, Emotion, Qual y Rating. Respecto a características específicas del indivudo se eligen: Mujer y Edad.
...
Para calcular la probabilidad de que una mujer de 25 años elija el anime Naruto se debe evaluar la función de utilidad para cada anime, de la forma: $U_{ik} = X_i \beta_{k}$ . No consideramos la información específica de cada capítulo, pues en este caso solo sabemos a priori que es una mujer de 25 años, por lo que consideramos el resto constante y no existen diferencias relativas. Se debe tener en cuenta que esto se realiza en base a un caso de control, siento en este caso el anime Boku No Hero, este anime se considera que tiene $U_{i,bnh} = 0$ . Luego de calcular cada función de utilidad la fórmula específica para obtener la probabilidad de Naruto se obtiene de:

$$
p_{i,naruto} = \frac{e^{U_{i,naruto}}}{\sum_{k \in Eleccion} e^{U_{ik}}}
$$

```{r probabilidad mujer 25 años}
...
A continuación se estudia el modelo mixed Logit donde se asume una distribución sobre las probabilidades de elegir cada alternativa. En este asumimos que las distribuciones de los coeficientes asociados a variables que cambian según la alternativa tendrán una distribución Normal, donde el parámetro a estimar será su media y desviación estándar, es decir, para cada parámetro tenemos una distribución: $$\alpha \sim N(b,V_b)$$.
